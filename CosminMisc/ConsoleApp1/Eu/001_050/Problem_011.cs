...
namespace ConsoleApp1.Eu._001_050
{
    ...
    {
        readonly static string _dataStr =
@"
...
";
        readonly static int _matrixSize = 20;
        readonly static int _factorCount = 4;
        ...
        private static void ComputeColumnMaxProduct(byte[][] data, ref long maxProduct)
        {
            ...
            {
                byte[] columnData = new byte[data.Length];
                for (int i = 0; i < data.Length; i++)
        ...
        private static void ComputeDiagNWSEMaxProduct(byte[][] data, ref long maxProduct)
        {
            ...
            {
                int diagLength = data.Length - i;
                byte[] diagData = new byte[diagLength];
            ...
            // top right half
            for (int j = 1; j < data.Length - _factorCount; j++)
            {
                int diagLength = data.Length - j;
                byte[] diagData = new byte[diagLength];
        ...
        {
            byte[][] data = new byte[_matrixSize][];
            ...
            {
                byte[] numbers = line.Split(' ').Select(a => byte.Parse(a)).ToArray();
                data[lineIndex++] = numbers;
            }
